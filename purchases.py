...
#     lblInfo=Label(font=('arial',20,'bold'),text=localtime,fg="Steel Blue",bd=10,anchor='w')
...
class Bill_App:
        ...
        def __init__(self,root):
                ...
                lbl_color = 'white'
                #Title of App
                title = Label(self.root,text = "Purchase For" + " " + localtime ,bd = 12,relief = GROOVE,fg = fg_color,bg = bg_color,font=("times new roman",30,"bold"),pady = 3).pack(fill = X)
                ...
                #===========
                bill_title = Label(F3,text = "Purchased Items",font = ("Lucida",13,"bold"),bd= 7,relief = GROOVE)
        ...
        def welcome_soft(self):
                ...
                self.txt.insert(END,"       Welcome To Tunmmy Retail\n")
        ...
        def bill_area(self):
                ...
                self.txt.insert(END,"\n===================================")
                Final_price = self.txt.insert(END,f"\n                      Total : {self.total_cosmetics_prices+self.total_grocery_prices+self.total_other_prices+self.total_cosmetics_prices * 0.05+self.total_grocery_prices * 0.05+self.total_other_prices * 0.05}")
                total_sum = self.total_cosmetics_prices+self.total_grocery_prices+self.total_other_prices+self.total_cosmetics_prices * 0.05+self.total_grocery_prices * 0.05+self.total_other_prices * 0.05
                print(total_sum)
                ...
                cur = con.cursor()
                uuid = str(uuid4())
                # Should only run once on first creation
               #  cur.execute('''CREATE TABLE Sap 
                # (
                # ID     TEXT PRIMARY KEY     NOT NULL,
                # TOTAL FLOAT NULL
                # );''')

                cur.execute('INSERT INTO Sap (ID,TOTAL) VALUES (%s, %s)',
                (uuid,total_sum))
